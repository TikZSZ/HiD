...
const IssueVCModal: React.FC<{
  ...
  const handleFormSubmit = (data: IssueVCFormValues) => {
    issueVCMutation.mutate(data);
  };

  const validateForm = async (data: IssueVCFormValues) => {
    const didDocument = await resolveDID(data.identifier)
    if(didDocument && !didDocument.hasOwner() ) form.setError("identifier",{message:"Invalid DIDIdentifier"})
  ...
  return (
    ...
    >
      <Form {...form}>
        <form onSubmit={form.handleSubmit(validateForm)} className="space-y-4">
          {/* Identifier Field */}
          <FormField
            name="identifier"
