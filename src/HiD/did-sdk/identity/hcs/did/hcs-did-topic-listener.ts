...
import { MessagesResponse, Client as MirrorClient, topicMessages } from '@tikz/hedera-mirror-node-ts';
...
export class HcsDidTopicListener
{
    ...
    public subscribeRest ( receiver: ( input: MessageEnvelope<HcsDidMessage> ) => void ): HcsDidTopicListener
    {
        class ITopicMessage
        {
            ...
            chunks:any[]=[]
            constructor( response: MessagesResponse[ "messages" ] extends ( infer U )[] ? U : never )
            {
                this.consensusTimestamp = Timestamp.fromDate(response.consensus_timestamp);
                this.message = Buffer.from( response.message, "base64" );
                this.contents = Buffer.from( response.message, "base64" );
                this.runningHash = Buffer.from( response.running_hash, "base64" );
                this.initialTransactionId = response.chunk_info.initial_transaction_id
                this.sequenceNumber = Long.fromNumber(response.sequence_number);
            }
        }

        ( async () =>
        {
            const msgCursor = topicMessages( mirrorClient ).setTopicId( this.topicId.toString() ).setLimit( 100 ).order( "asc" )
            let messages =  ( await msgCursor.get() ).messages
            let shouldFetchMore = true
            while ( shouldFetchMore )
            {
                let nextMessagesSet =  ( await msgCursor.next() )
                if ( !nextMessagesSet )
                {
                    shouldFetchMore = false
                } else
                {
                    [ messages, ...nextMessagesSet.messages ]
                }
            }
            messages.map( ( msg ) =>
            {
                return this.handleResponse( new ITopicMessage(msg), receiver );
            } )
        } )()
        return this;
    }
